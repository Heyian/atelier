...
> **Last verified 2026-08-10** — source: Anthropic help center, article 12512180
> ("Utiliser les compétences dans Claude"). Capabilities shift month to month:
> show the executive this date, and offer to re-verify against
...
on the rest of the job — Chat for a quick question, Cowork if it touches
files. But if the work depends on automation (hooks, sub-agents), only
Cowork runs it; elsewhere it shows up greyed out, not missing.

## Try it on your own

Open Customize in your claude.ai account settings, and look at the Skills
and Connectors sections: which skills are turned on, which connectors are
attached. That's where you flip either one — mentor can't do it for you.
